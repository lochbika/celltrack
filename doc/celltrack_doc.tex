...
The detection of subcells and the linking to cells is can also handle periodic boundaries.
...
\begin{labeling}{-maxnants}
	...
	\item[-nants] Number of ants/agents for mainstream detection. Default value is the number of nodes of a meta track.
	\item[-maxnants] The maximum number of ants/agents for mainstream detection. Setting this value can be useful to avoid long computation times for large meta track. Nevertheless it may decrease the validity of the detected mainstream.
    ...
    \item[-maxv] The maximum allowed velocity of cells. This prevents that errors in the advection correction (unrealistically high velocities) are taken into account. Default is 50 distance units/per second.
    ...
    \item[-minarea] The minimum area of a cluster in grid points. Smaller clusters will be omitted. Default is 0.
